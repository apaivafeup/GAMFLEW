    ...
    <div class="container">
                ...
                <div class="col" style="margin-bottom: 10px; margin-left: 7.5px; border: #ccc 1px solid; padding: 10px;">
                    ...
                    <div class="row" style="display: flex; justify-content: center; margin-top: 10px; margin-bottom: 10px;">
                        <div class="game-board-out-labels">
                            <div class="game-board-label col" style="display: flex; justify-content: center; font-size: 16px;">Out</div>
                          </div>
                          <div class="game-board-out">
                            ...
                            <div style="width: 100%; display: flex; flex-direction: row; justify-content: center">
                              <input id="piece-stack-out-x"
                               class="col box disabled" style="width: 30px; text-align: center; font-size: 12px" type="number" />
                              <input id="piece-stack-out-y"
                                class="col box disabled" style="width: 30px; text-align: center; font-size: 12px" type="number" />
                            </div>
                          </div>
                    </div>
                    <p style="text-align: justify; margin: 0px">
                        You can use the two input boxes below to provide <em>(x, y)</em> coordinates of where to move. The piece will be moved to the spot with the given coordinates.
                        <strong>When the spot is occupied,</strong> the input boxes are disabled. Move it out of the way to write in them again.
                    </p>
                </div>
            </div>
        </div>
        <div v-if="tab == 'auxiliary_functions'" style="font-size: 13px !important">
            ...
            <div class="row" style="padding: 10px">
                <button :class="name == 'isTriangle'
                    ? 'col button is-primary is-fullwidth guide-button guide-button-selected'
                    : 'col button is-primary is-fullwidth guide-button'
                    " :id="'guide-button-' + name" v-for="name in auxiliaryFunctionsNames"
                    @click="selectFunction(name)">
                    {{ name }}
                </button>
            </div>
            <div class="row">
                <CodeBlock id="code-block-example" class="line-numbers" theme="default" height="314.5px" data-line="1"
                    :prismjs="true" :name="name" :code="auxiliaryFunctions[selectedFunction]" lang="javascript" prism-js
                    style="font-size: 16px; overflow: scroll" :copy-icon="false" :copy-button="false" :copy-tab="false"
                    :tabs="false" />
            </div>
        </div>
